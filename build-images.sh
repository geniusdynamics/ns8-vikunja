...
reponame="vikunja"
vikunja_version="1.1.0"
# Create a new empty container image
container=$(buildah from scratch)

# Reuse existing nodebuilder-vikunja container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-vikunja; then
	echo "Pulling NodeJS runtime..."
	buildah from --name nodebuilder-vikunja -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
	...
	--label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/vikunja/vikunja:$vikunja_version" \
	"${container}"
# Commit the image
